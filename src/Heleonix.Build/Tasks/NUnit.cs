...
public class NUnit : BaseTask
{
    ...
    {
        Prepare(this.TestResultFile, this.TestOutputFile);

        var args = ArgsBuilder.By("--", "=")
            .AddPaths(this.NUnitProjectFileOrTestFiles.Select(i => i.ItemSpec))
            .AddPath("result", this.TestResultFile)
            .AddPath("testlist", this.TestListFile)
            ...
            .AddPath("output", this.TestOutputFile)
        ...
        var testRun = XDocument.Load(this.TestResultFile).Element("test-run");

        this.TestCases = Convert.ToInt32(testRun.Attribute("testcasecount").Value, InvariantInfo);
        this.Total = Convert.ToInt32(testRun.Attribute("total").Value, InvariantInfo);
        this.Passed = Convert.ToInt32(testRun.Attribute("passed").Value, InvariantInfo);
        this.Failed = Convert.ToInt32(testRun.Attribute("failed").Value, InvariantInfo);
        this.Inconclusive = Convert.ToInt32(testRun.Attribute("inconclusive").Value, InvariantInfo);
        this.Skipped = Convert.ToInt32(testRun.Attribute("skipped").Value, InvariantInfo);
        this.Asserts = Convert.ToInt32(testRun.Attribute("asserts").Value, InvariantInfo);
